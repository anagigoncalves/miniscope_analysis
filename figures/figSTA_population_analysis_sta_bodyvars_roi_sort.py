...
for count_v, var in enumerate(var_names):
    ...
    ax[count_v].set_yticks(np.arange(0, np.shape(sta_zoom_all_concat_vars[count_v])[0], 50))
...
for count_v, var in enumerate(var_names):
    ...
    ax[count_v].set_xticklabels([str(xaxis[xaxis_start]), '0', str(np.round(xaxis[xaxis_end], 2))], fontsize=20)
    ax[count_v].axvline(x=np.where(xaxis==0)[0][0]-xaxis_start, color='white', linewidth=2)
    ax[count_v].set_yticks(np.arange(0, np.shape(sta_zoom_all_concat_vars_notzscored[count_v])[0], 50))
    ax[count_v].set_xlabel('Time around event (s)', fontsize=20)
    ax[count_v].tick_params(axis='both', which='major', labelsize=16)
    if sort_type == 'ML':
        ax[count_v].set_yticklabels(list(map(str, np.round(np.sort(sta_ml[::50]), 2))), fontsize=12, rotation=45)
    ...
    ax[count_v].set_title(var, fontsize=16)
plt.savefig(os.path.join(save_path,
                         'sta_bodyvars_' + load_path.split('\\')[-2].replace(' ','_') + '_animal_summary_notzscored_roi_sort_'+sort_type), dpi=mscope.my_dpi)
...
for count_v, var in enumerate(var_names):
    ...
    ax[count_v].set_xticklabels([str(xaxis[xaxis_start]), '0', str(np.round(xaxis[xaxis_end], 2))], fontsize=20)
    ax[count_v].axvline(x=np.where(xaxis==0)[0][0]-xaxis_start, color='white', linewidth=2)
    ax[count_v].set_yticks(np.arange(0, np.shape(sta_zoom_all_concat_vars_shuffled[count_v])[0], 50))
    ax[count_v].set_xlabel('Time around event (s)', fontsize=20)
    ax[count_v].tick_params(axis='both', which='major', labelsize=16)
    if sort_type == 'ML':
        ax[count_v].set_yticklabels(list(map(str, np.round(np.sort(sta_ml[::50]), 2))), fontsize=12, rotation=45)
    ...
    ax[count_v].set_title(var, fontsize=16)
plt.savefig(os.path.join(save_path,
                         'sta_bodyvars_' + load_path.split('\\')[-2].replace(' ','_') + '_animal_summary_shuffled_roi_sort_'+sort_type), dpi=mscope.my_dpi)

fig, ax = plt.subplots(1, len(var_names), figsize=(20, 10), tight_layout='True')
for count_v, var in enumerate(var_names):
    idx_notsig = np.where((np.array(sta_zoom_all_concat_vars[count_v]) < 2) & (np.array(sta_zoom_all_concat_vars[count_v]) > -2))
    sta_zoom_notzscore_sig = np.array(sta_zoom_all_concat_vars_notzscored[count_v]).copy()
    ...
    ax[count_v].set_title(var, fontsize=16)
plt.savefig(os.path.join(save_path,
                         'sta_bodyvars_' + load_path.split('\\')[-2].replace(' ','_') + '_animal_summary_notzscored_sig_roi_sort_'+sort_type), dpi=mscope.my_dpi)