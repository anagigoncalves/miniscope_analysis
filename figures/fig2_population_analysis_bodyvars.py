...
for var in var_names:
    ...
    for count_f, f in enumerate(animal_order):
        ...
        clusters_in_session = np.unique(coord_ext_overlap)
        for count_c, c in enumerate(clusters_in_session[1:]): #0 are ROIs that don't overlap with reference session
...
for count_v, var in enumerate(var_names):
    ...
    if sort_type == 'none':
        ax[count_v].set_ylabel('   '.join(sta_animal_id), fontsize=12)
