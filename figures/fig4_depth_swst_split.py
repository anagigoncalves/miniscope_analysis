...
# ROI map of depth of modulation first split trial - step cycle modulation
for paw in paws:
    fig, ax = plt.subplots(tight_layout=True, figsize=(5, 5))
    data_plot_tf = df_amp.loc[(df_amp['paw'] == paw) & (df_amp['trial'] == 7) & (df_amp['phase'] == 'cycle')]
    sc = ax.scatter(data_plot_tf['coord_AP'], data_plot_tf['coord_ML'], s=15, c=data_plot_tf['amp'],
                    cmap='viridis')
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    ax.tick_params(axis='both', which='major', labelsize=20)
    ...
    cbar.mappable.set_clim(0.5, 7)
    if save_fig:
        plt.savefig(os.path.join(save_path, 'firing_rate_amp_earlysplit_step_' + paw + '_roilocation'),
                    dpi=256)
        plt.savefig(os.path.join(save_path, 'firing_rate_amp_earlysplit_step_' + paw + '_roilocation.svg'),
                    dpi=256)
# ROI map of depth of modulation last split trial - step cycle modulation
for paw in paws:
    fig, ax = plt.subplots(tight_layout=True, figsize=(5, 5))
    data_plot_tf = df_amp.loc[(df_amp['paw'] == paw) & (df_amp['trial'] == 16) & (df_amp['phase'] == 'cycle')]
    sc = ax.scatter(data_plot_tf['coord_AP'], data_plot_tf['coord_ML'], s=15, c=data_plot_tf['amp'],
                    cmap='viridis')
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    ...
    cbar.mappable.set_clim(0.5, 7)
    if save_fig:
        plt.savefig(os.path.join(save_path, 'firing_rate_amp_latesplit_step_' + paw + '_roilocation'),
                    dpi=256)
        plt.savefig(os.path.join(save_path, 'firing_rate_amp_latesplit_step_' + paw + '_roilocation.svg'),
                    dpi=256)
# ROI map of depth of modulation first washout trial - step cycle modulation
for paw in paws:
    fig, ax = plt.subplots(tight_layout=True, figsize=(5, 5))
    data_plot_tf = df_amp.loc[(df_amp['paw'] == paw) & (df_amp['trial'] == 17) & (df_amp['phase'] == 'cycle')]
    sc = ax.scatter(data_plot_tf['coord_AP'], data_plot_tf['coord_ML'], s=15, c=data_plot_tf['amp'],
                    cmap='viridis')
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    ...
    cbar.mappable.set_clim(0.5, 7)
    if save_fig:
        plt.savefig(os.path.join(save_path, 'firing_rate_amp_earlywashout_step_' + paw + '_roilocation'),
                    dpi=256)
        plt.savefig(os.path.join(save_path, 'firing_rate_amp_earlywashout_step_' + paw + '_roilocation.svg'),
                    dpi=256)
plt.close('all')
...
# # Do also line plots of mean and std - learning style - sw and st modulation
# for paw in paws:
#     fig, ax = plt.subplots(figsize=(5, 7), tight_layout=True)
...
#     ax[0].set_xticks([0, 1])
#     ax[0].set_xticklabels(['last baseline', 'early split'])
...
for paw in paws:
    ...
    delta_amp_st = np.array(data_plot_st_tf['amp'])
    ...
    cbar.ax.tick_params(labelsize=20)
    if save_fig:
        plt.savefig(os.path.join(save_path, 'firing_rate_amp_latesplit_st_' + paw + '_roilocation'),
                    dpi=256)
        plt.savefig(os.path.join(save_path, 'firing_rate_amp_latesplit_st_' + paw + '_roilocation.svg'),
                    dpi=256)
    fig, ax = plt.subplots(tight_layout=True, figsize=(5, 5))
    data_plot_sw_tf = df_amp.loc[(df_amp['paw'] == paw) & (df_amp['trial'] == 16) & (df_amp['phase'] == 'sw')]
    delta_amp_sw = np.array(data_plot_sw_tf['amp'])
    ...
    cbar.ax.tick_params(labelsize=20)
    if save_fig:
        plt.savefig(os.path.join(save_path, 'firing_rate_amp_latesplit_sw_' + paw + '_roilocation'),
                    dpi=256)
        plt.savefig(os.path.join(save_path, 'firing_rate_amp_latesplit_sw_' + paw + '_roilocation.svg'),
                    dpi=256)
    plt.close('all')