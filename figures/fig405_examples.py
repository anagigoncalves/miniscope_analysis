...
sta_zs = np.load(
    os.path.join(load_path, animal + ' split contra fast', 'sta_bodyvars_' + var_name.replace(' ', '_') + '_zscored.npy'))
...
for t in range(len(cond_name)):
    ax.plot(xaxis[xaxis_start:xaxis_end], sta_zs_zoom[roi_plot, t, :],
...
sta_zs_405 = np.load(
    os.path.join(load_path, animal + ' split contra fast 405', 'sta_bodyvars_' + var_name.replace(' ', '_') + '_zscored.npy'))
...
trials_ses_405 = np.array([[ 1,  6],
                            ...
                            [17, 26]])
if protocol_type == 'tied':
    sta_zs_zoom_405 = np.zeros((np.shape(sta_zs)[0], len(cond_name), xaxis_end - xaxis_start))
    ...
    for count_c, c in enumerate(trials_ses_name):
        if trials_ses_name_405[count_c] == 'baseline speed':
            ...
            sta_zs_zoom_405[:, 1, :] = np.nanmean(sta_zs_405[:, trial_start_idx:trial_end_idx, xaxis_start:xaxis_end], axis=1)
        if trials_ses_name_405[count_c] == 'slow speed':
            bs_idx = trials_ses_name_405.index('slow speed')
            trial_start_idx = trials_idx_405[np.where(trials_405 == trials_ses_405[bs_idx, 0])[0][0]]
            trial_end_idx = trials_idx_405[np.where(trials_405 == trials_ses_405[bs_idx, 1])[0][0]]
            sta_zs_zoom_405[:, 0, :] = np.nanmean(sta_zs_405[:, trial_start_idx:trial_end_idx, xaxis_start:xaxis_end], axis=1)
        if trials_ses_name_405[count_c] == 'fast speed':
            bs_idx = trials_ses_name_405.index('fast speed')
            trial_start_idx = trials_idx_405[np.where(trials_405 == trials_ses_405[bs_idx, 0])[0][0]]
            trial_end_idx = trials_idx_405[np.where(trials_405 == trials_ses_405[bs_idx, 1])[0][0]]
            sta_zs_zoom_405[:, 2, :] = np.nanmean(sta_zs_405[:, trial_start_idx:trial_end_idx, xaxis_start:xaxis_end], axis=1)
if protocol_type == 'split':
    sta_zs_zoom_405 = np.zeros((np.shape(sta_zs_405)[0], len(cond_name), xaxis_end - xaxis_start))
    sta_zs_zoom_405[:] = np.nan
    ...
    for count_c, c in enumerate(trials_ses_name_405):  # if odd is -1, if even is the next
        ...
        sta_zs_zoom_405[:, count_c, :] = np.nanmean(sta_zs_405[:, trial_start_idx:trial_end_idx, xaxis_start:xaxis_end], axis=1)
