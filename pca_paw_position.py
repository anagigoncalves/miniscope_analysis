...
# Load data
data_path = 'C:\\Users\\User\\Carey Lab Dropbox\\Rotation Carey\\Francesco and Ana G\\Miniscope processed files\\Behavior\\Paw position all animals split contra fast.npy'
...
for animal in animals:
    ...
    for trial_idx, trial in enumerate(trials):
        principal_components[f'{trial}'] = pca.transform(paws_position.iloc[np.where(trial_id == trial)[0]])
    ...
    for trial_idx, trial in enumerate(trials):
        ...
        global_phase[f'{trial}'] = np.mod(global_phase_unwrapped.iloc[0, :], 2 * np.pi).values # wrap phase
        peaks, troughs = findpeaks(global_phase[f'{trial}'], peak_threshold=6.1, trough_threshold=0.3)
    ...
    # Interpolate cycles in the behavioral manifold and average them    
        ...
        for i in range(len(peaks)):
            ...
            cycles.append(principal_components[f'{trial}'][onset:offset+1, :])
        interp_cycles[f'{trial}'] = interpolate_arrays(cycles, target_length=200)
        avg_trajectory[f'{trial}'] = [np.mean([cycle[:, col] for cycle in interp_cycles[f'{trial}']], axis = 0) for col in range(3)]
    
    # Plot example of global phase
    plt.figure(figsize=(12, 6))
    plt.subplot(1, 2, 1)
    plt.plot(np.arange(len(global_phase['1'][200:2000] - 1)) / sr_cam, global_phase['1'][200:2000], color = 'dimgray')
    ...
    for trial_idx, trial in enumerate(trials):
        ...
        ax.plot(
            principal_components[f'{trial}'][:, 0],
            principal_components[f'{trial}'][:, 1],
            principal_components[f'{trial}'][:, 2],
            color = 'dimgray', alpha = 0.3, linewidth = 0.5)
        ax.set_xlabel('PC1', fontsize=15)
        ax.set_ylabel('PC2', fontsize=15)
        ax.set_zlabel('PC3', fontsize=15)
        ax.plot(avg_trajectory[f'{trial}'][0], avg_trajectory[f'{trial}'][1], avg_trajectory[f'{trial}'][2], color = color_trials[trial], linewidth = 2)
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_zticks([])
        ax.set_xticklabels([])
    ...
    colors = colormap(normalize(global_phase['2']))
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    scatter = ax.scatter(
        principal_components['2'][:, 0],
        principal_components['2'][:, 1],
        principal_components['2'][:, 2],
        color = colors, s = 1)
    cbar = plt.colorbar(scatter, orientation='vertical')
    cbar.set_label('Global phase', fontsize=15)
    ax.set_xlabel('PC1', fontsize=15)
    ax.set_ylabel('PC2', fontsize=15)
    ax.set_zlabel('PC3', fontsize=15)
    ...
    traj_speed = np.diff(principal_components['2'], axis=0) / (1/sr_cam)
    ...
    for p, paw in enumerate(paws):
        st_idx_paw = np.array(st_idx[paw][6]).astype(int)
        scatter = ax.scatter(
            principal_components['7'][:, 0],
            principal_components['7'][:, 1],
            principal_components['7'][:, 2],
            color = 'lightgray', alpha = 0.1,  s = 0.5)
        scatter = ax.scatter(
            principal_components['7'][st_idx_paw, 0],
            principal_components['7'][st_idx_paw, 1],
            principal_components['7'][st_idx_paw, 2],
    ...
    for p, paw in enumerate(paws):
        ...
        scatter = ax.scatter(
            ...
            color = 'lightgray', alpha = 0.1,  s = 0.5)
        scatter = ax.scatter(
            principal_components['2'][sw_idx_paw, 0],
            principal_components['2'][sw_idx_paw, 1],
            principal_components['2'][sw_idx_paw, 2],
            color = paw_color[p], s = 30, marker = '.', zorder = 50)
    ax.set_xlabel('PC1', fontsize=15)
    ax.set_ylabel('PC2', fontsize=15)
    ax.set_zlabel('PC3', fontsize=15)
    ...
    # Plot distribution of swing and stance with respect to the global phase
    sw_phase = {}
    st_phase = {}
    for p, paw in enumerate(paws):
        sw_phase[paw] = [global_phase[f'{trial}'][np.array(sw_idx[paw][trial_idx]).astype(int)] for trial_idx, trial in enumerate(trials)]
        st_phase[paw] = [global_phase[f'{trial}'][np.array(st_idx[paw][trial_idx]).astype(int)] for trial_idx, trial in enumerate(trials)] 
    for trial_idx, trial in enumerate(trials):
    ...
    for pc in range(3):
        for trial_idx, trial in enumerate(trials):
            axes[pc].plot(avg_trajectory[f'{trial}'][pc], color = color_trials[trial], linewidth = 2)
    ...
    # Plot euclidean distance from manifold's center of mass for each trial
    fig, ax = plt.subplots(subplot_kw={'projection': 'polar'})
    for trial_idx, trial in enumerate(trials):
        center = np.mean(principal_components[f'{trial}'], axis=0)
        center_distance = np.mean([[np.sqrt(np.sum(((sample - center)**2))) for sample in cycle] for cycle in interp_cycles[f'{trial}']], axis=0)
    ...
    dist2avg_mean = []
    for trial in trials:
        dist2avg = [np.linalg.norm(cycle.T - np.array(avg_trajectory[f'{trial}']), axis = 0) for cycle in interp_cycles[f'{trial}']]
    ...
    rotation_angle = []
    for trial_idx, trial in enumerate(trials):
        rotation_angle.append(get_rotation_angle(np.array(avg_trajectory[f'{trial}']), np.array(avg_trajectory[f'{1}'])))
