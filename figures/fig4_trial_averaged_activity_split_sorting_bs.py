...
for count_a, animal in enumerate(animals):
    ...
    if (protocol == 'split ipsi fast' and animal == 'MC8855') or (protocol == 'split contra fast' and animal == 'MC8855'):
        firing_rate_mean_trials_paw_bs.append(np.nanmean(firing_rate_animal[:, 0, :3, :], axis=1))
    else:
        firing_rate_mean_trials_paw_bs.append(np.nanmean(firing_rate_animal[:, 0, :6, :], axis=1))
...
for count_a, animal in enumerate(animals):
    firing_rate_animal = np.load(os.path.join(load_path, animal + ' ' + protocol, 'raster_firing_rate_rois.npy'))
    if (protocol == 'split ipsi fast' and animal == 'MC8855') or (protocol == 'split contra fast' and animal == 'MC8855'):
        # firing_rate_mean_trials_paw_es.append(firing_rate_animal[:, 0, 3, :])
        firing_rate_mean_trials_paw_es.append(np.nanmean(firing_rate_animal[:, 0, [3, 4], :], axis=1))
    else:
        # firing_rate_mean_trials_paw_es.append(firing_rate_animal[:, 0, 6, :])
...
for count_a, animal in enumerate(animals):
    firing_rate_animal = np.load(os.path.join(load_path, animal + ' ' + protocol, 'raster_firing_rate_rois.npy'))
    if (protocol == 'split ipsi fast' and animal == 'MC8855') or (protocol == 'split contra fast' and animal == 'MC8855'):
        # firing_rate_mean_trials_paw_ls.append(firing_rate_animal[:, 0, 12, :])
        ...
        # firing_rate_mean_trials_paw_ls.append(firing_rate_animal[:, 0, 13, :])
        firing_rate_mean_trials_paw_ls.append(np.nanmean(firing_rate_animal[:, 0, [12, 13], :], axis=1))
    else:
        #firing_rate_mean_trials_paw_ls.append(firing_rate_animal[:, 0, 14, :])
...
for count_a, animal in enumerate(animals):
    firing_rate_animal = np.load(os.path.join(load_path, animal + ' ' + protocol, 'raster_firing_rate_rois.npy'))
    if (protocol == 'split ipsi fast' and animal == 'MC8855') or (protocol == 'split contra fast' and animal == 'MC8855'):
        # firing_rate_mean_trials_paw_ae.append(firing_rate_animal[:, 0, 13, :])
        firing_rate_mean_trials_paw_ae.append(np.nanmean(firing_rate_animal[:, 0, [13, 14], :], axis=1))
    elif protocol == 'split contra fast' and animal == 'MC10221':
        # firing_rate_mean_trials_paw_ae.append(firing_rate_animal[:, 0, 15, :])
        firing_rate_mean_trials_paw_ae.append(np.nanmean(firing_rate_animal[:, 0, [15, 16], :], axis=1))
    else:
        # firing_rate_mean_trials_paw_ae.append(firing_rate_animal[:, 0, 16, :])
...
for count_a, animal in enumerate(animals):
    firing_rate_animal = np.load(os.path.join(load_path, animal + ' ' + protocol, 'raster_firing_rate_rois.npy'))
    if (protocol == 'split ipsi fast' and animal == 'MC8855') or (protocol == 'split contra fast' and animal == 'MC8855'):
        # firing_rate_mean_trials_paw_lw.append(firing_rate_animal[:, 0, 22, :])
        ...
        # firing_rate_mean_trials_paw_lw.append(firing_rate_animal[:, 0, 24, :])
        firing_rate_mean_trials_paw_lw.append(np.nanmean(firing_rate_animal[:, 0, [23, 24], :], axis=1))
    else:
        # firing_rate_mean_trials_paw_lw.append(firing_rate_animal[:, 0, 25, :])
...
fig, ax = plt.subplots(tight_layout=True, figsize=(6, 5))
hm = sns.heatmap(firing_rate_mean_trials_paw_concat_es[np.argsort(np.argmax(firing_rate_mean_trials_paw_concat_bs, axis=1))],
        ax=ax, cmap='viridis')
cbar = hm.collections[0].colorbar
cbar.ax.tick_params(labelsize=18)
ax.set_xticks(np.linspace(0, 20, 10))
...
fig, ax = plt.subplots(tight_layout=True, figsize=(6, 5))
hm = sns.heatmap(firing_rate_mean_trials_paw_concat_ls[np.argsort(np.argmax(firing_rate_mean_trials_paw_concat_bs, axis=1))],
        ax=ax, cmap='viridis')
cbar = hm.collections[0].colorbar
cbar.ax.tick_params(labelsize=18)
ax.set_xticks(np.linspace(0, 20, 10))
...
fig, ax = plt.subplots(tight_layout=True, figsize=(6, 5))
hm = sns.heatmap(firing_rate_mean_trials_paw_concat_ae[np.argsort(np.argmax(firing_rate_mean_trials_paw_concat_bs, axis=1))],
        ax=ax, cmap='viridis')
cbar = hm.collections[0].colorbar
cbar.ax.tick_params(labelsize=18)
ax.set_xticks(np.linspace(0, 20, 10))
...
fig, ax = plt.subplots(tight_layout=True, figsize=(6, 5))
hm = sns.heatmap(firing_rate_mean_trials_paw_concat_lw[np.argsort(np.argmax(firing_rate_mean_trials_paw_concat_bs, axis=1))],
        ax=ax, cmap='viridis')
cbar = hm.collections[0].colorbar
cbar.ax.tick_params(labelsize=18)
ax.set_xticks(np.linspace(0, 20, 10))
