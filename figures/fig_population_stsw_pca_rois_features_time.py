...
# Input data
load_path_st = 'J:\\Miniscope processed files\\Analysis on population data\\Rasters 20 bins\\Rasters st time\\tied baseline S1\\'
load_path_sw = 'J:\\Miniscope processed files\\Analysis on population data\\Rasters 20 bins\\Rasters sw time\\tied baseline S1\\'
...
for p in range(len(paws)):
    ...
    for count_a, animal in enumerate(animals):
        ...
        firing_rate_mean_trials_paw_st.append(zscoring(np.nanmean(firing_rate_animal_st[:, p, :, :], axis=1).T, 0))
        firing_rate_mean_trials_paw_sw.append(zscoring(np.nanmean(firing_rate_animal_sw[:, p, :, :], axis=1).T, 0))
        firing_rate_mean_trials_paw_animalid.append(np.repeat(count_a, np.shape(firing_rate_animal_st)[1]))
    firing_rate_mean_trials_paw_concat_st = np.hstack(firing_rate_mean_trials_paw_st)
    firing_rate_mean_trials_paw_concat_sw = np.hstack(firing_rate_mean_trials_paw_sw)
    # Paw concatenation FR x (timexpaws)
    if p == 0:
        firing_rate_animal_trials_concat_paws_st = firing_rate_mean_trials_paw_concat_st
        firing_rate_animal_trials_concat_paws_sw = firing_rate_mean_trials_paw_concat_sw
    else:
        firing_rate_animal_trials_concat_paws_st = np.concatenate(
            (firing_rate_animal_trials_concat_paws_st, firing_rate_mean_trials_paw_concat_st), axis=0)
        firing_rate_animal_trials_concat_paws_sw = np.concatenate(
            (firing_rate_animal_trials_concat_paws_sw, firing_rate_mean_trials_paw_concat_sw), axis=0)
...
firing_rate_animal_trials_concat_paws_stsw = np.vstack((firing_rate_animal_trials_concat_paws_st, firing_rate_animal_trials_concat_paws_sw))
comp = 9
pca_fr_paws = PCA(n_components=comp)
pca_fit_fr_paws = pca_fr_paws.fit(firing_rate_animal_trials_concat_paws_stsw)
pca_fit_fr_paws_fit_transform = pca_fr_paws.fit_transform(firing_rate_animal_trials_concat_paws_stsw)
...
pca_fit_fr_paws_models = pca_fr_paws.fit(firing_rate_animal_trials_concat_paws_stsw)
data_PCA = pca_fit_fr_paws_models.transform(firing_rate_animal_trials_concat_paws_stsw)
pca_fit_fr_single_paws = np.reshape(data_PCA.T, ((8, 4, len(bins)-1)))
...
for c in range(8):
    if c%2 == 0:
        for count_p in range(len(paws)):
            ax[0, count_even].plot(bins_fr[:-1], pca_fit_fr_single_paws[c, count_p, :], color=paw_colors[count_p], linewidth=3)
    ...
    if c%2 != 0:
        for count_p in range(len(paws)):
            ax[1, count_odd].plot(bins_fr[:-1], pca_fit_fr_single_paws[c, count_p, :], color=paw_colors[count_p], linewidth=3)
        ...
        count_odd += 1
plt.savefig(os.path.join(save_path, 'pca_mean_firingrate_' + align_dimension + '_temporaldimension'), dpi=256)
plt.savefig(os.path.join(save_path, 'pca_mean_firingrate_' + align_dimension + '_temporaldimension.svg'), dpi=256)

# First PCs contribution to each ROI
for c in range(4):
    fig, ax = plt.subplots(tight_layout=True, figsize=(5, 5))
    sc = ax.scatter(roi_coordinates_arr[:, 0], roi_coordinates_arr[:, 1], s=15, c=pca_fit_fr_paws_models.components_[c, :], cmap='coolwarm')
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    ax.tick_params(axis='both', which='major', labelsize=20)
    ...
    plt.savefig(os.path.join(save_path, 'pca_mean_firingrate_' + align_dimension + '_pc' + str(c+1) + '_roilocation'), dpi=256)
    plt.savefig(os.path.join(save_path, 'pca_mean_firingrate_' + align_dimension + '_pc' + str(c+1) + '_roilocation.svg'), dpi=256)
