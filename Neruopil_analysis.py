...
# new_columns = time_info+df_session.columns.to_list()
# trials_df = pd.read_csv(os.path.join(path_results,'df_extract_raw_split.csv'), usecols=['time','trial'])
# zscored_neuropil_signal= pd.concat([trials_df, df_session], axis=1, ignore_index=True)
...
# df_session = pd.read_csv(os.path.join(path_results,'neuropil_session_signal.csv'), index_col=0) #OLD
# zscored_neuropil_signal = neuropil.norm_traces(df_session,df_session.columns) #OLD
# zscored_neuropil_signal.to_csv(os.path.join(path_results,'neuropil_session_signal_norm.csv')) #OLD
#  '''
...
selected_clusters = [1,2,3,4,7,12,13,14,16,18]
nr_clusters = len(cluster_idx_list)
...
cluster_idx_list = np.unique(cluster_idx['cluster_idx'].to_numpy())
selected_clusters = cluster_idx_list
trials_df = pd.read_csv(os.path.join(path_results,'df_extract_raw_split.csv'), usecols=['trial','time'])
trials = trials_df['trial'].to_numpy()
...
for t in np.unique(trials):
    ...
    for i,c in enumerate(selected_clusters):
        ...
        ax_mean.legend()
        mean_neuropil_signal.append(mean_cluster_activity)
    fig_mean.savefig(os.path.join(path_results,'T'+str(t)+'_neuropil','cluisters_trace.png'))
    plt.close(fig_mean)
#  '''
...
for trial in range(1,n_trials+1):
    ...
    rois.to_csv(os.path.join(path_results,'T'+str(trial)+'_neuropil','T'+str(trial)+'_df_rois_raw_split_zscored.csv'))

rois_session_signal = []

for trial in range(1,n_trials+1):
    print('trial number: ', trial)
    rois_trial_signal = pd.read_csv(os.path.join(path_results,'T'+str(trial)+'_neuropil','T'+str(trial)+'_df_rois_raw_split_zscored.csv'))
    rois_session_signal.append(rois_trial_signal)    

df_session = pd.concat(rois_session_signal)
print(df_session.columns)
df_session.drop("Unnamed: 0", axis=1, inplace=True)
df_session.to_csv(os.path.join(path_results,'rois_session_signal_zscored_new.csv'))
#  '''
...
for j,c in enumerate(cluster_idx_list):
    ...
    print('Plotting  cluster ', c, ' activity')
    ax.plot(t,mean_cluster_activity+j*k,color=cmap.colors[c], label='cluster'+str(c))
    ax.hlines(j*k,t[0],t[-1], color=cmap.colors[c])
    # ax.fill_between(t,(mean_cluster_activity+j*k)+sd_cluster_activity,(mean_cluster_activity+j*k)-sd_cluster_activity,color=cmap.colors[c], alpha=0.8)
    ax.legend()
for t in np.unique(trials):
    ax.vlines(trials_df.index[trials_df['trial']==t][0], 0, j*k, linewidth=2)
# plt.show()
fig.savefig(os.path.join(path_results,'clustered_rois_trace.png'))
plt.close(fig)
...
for trial in range(1,n_trials+1):
    ...
    for i,c in enumerate(selected_clusters):
        ...
        ax_mean.legend()
    fig_mean.savefig(os.path.join(path_results,'T'+str(trial)+'_neuropil','T'+str(trial)+'_clustered_rois_trace.png'))
    # plt.show()
    plt.close(fig_mean)
