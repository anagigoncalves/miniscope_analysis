...
# Do also line plots of single rois per animal - step cycle modulation
for paw in paws:
...
for paw in paws:
    ...
    plt.gca().add_patch(rectangle)
    sw_st_idx = np.where((df_amp['phase']=='st')|((df_amp['phase']=='sw')))[0]
    df_amp_swst = df_amp.iloc[sw_st_idx, :]
    data_plot = df_amp_swst.loc[(df_amp_swst['paw']==paw)&(df_amp_swst['trial'].isin(np.array([1, 6, 7, 16, 17, 26])))]
    ...
    ax.set_ylabel('Event rate amplitude', fontsize=20)
    if save_fig:
        plt.savefig(os.path.join(save_path, 'firing_rate_amp_dist_' + paw),
                    dpi=256)
        plt.savefig(os.path.join(save_path, 'firing_rate_amp_dist_' + paw + '.svg'),
                    dpi=256)
...
for paw in paws:
    ...
    rectangle = plt.Rectangle((6.5, 0), 10, 3, fc='lightgrey', alpha=0.3, zorder=-1)
    plt.gca().add_patch(rectangle)
