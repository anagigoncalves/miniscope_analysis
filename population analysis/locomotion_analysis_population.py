...
main_control_path = 'E:\\Miniscope processed files\\Non-miniscope locomotion data\\\Treadmill tied\\gait parameters\\'
...
for count_p, g in enumerate(param_tied):
    ...
    ax[count_p].plot(df_intralimb_plot_animal_mean_speed[0, :], np.nanmean(df_intralimb_plot_animal_mean_values, axis=0), color='teal', linewidth=2)
    ax[count_p].fill_between(df_intralimb_plot_animal_mean_speed[0, :],
                     np.nanmean(df_intralimb_plot_animal_mean_values, axis=0)-np.nanstd(df_intralimb_plot_animal_mean_values, axis=0),
    np.nanmean(df_intralimb_plot_animal_mean_values, axis=0)+np.nanstd(df_intralimb_plot_animal_mean_values, axis=0), color='teal', alpha=0.3)
    ...
    ax[count_p].spines['right'].set_visible(False)
    ax[count_p].spines['top'].set_visible(False)
plt.savefig('E:\\Miniscope processed files\\tied belt locomotion analysis\\param_intralimb.png')
...
plt.savefig('E:\\Miniscope processed files\\tied belt locomotion analysis\\phase_st.png')
...
ax[0].plot(np.linspace(0, 100, 100), np.nanmean(swing_inst_vel_control_mat, axis=0), color='teal', linewidth=2)
ax[0].fill_between(np.linspace(0, 100, 100),
                 np.nanmean(swing_inst_vel_all_mat, axis=0)-np.nanstd(swing_inst_vel_all_mat, axis=0),
np.nanmean(swing_inst_vel_all_mat, axis=0)+np.nanstd(swing_inst_vel_all_mat, axis=0), color='teal', alpha=0.3)
...
ax[1].plot(np.linspace(0, 100, 100), np.nanmean(swing_z_all_mat, axis=0), color='teal', linewidth=2)
ax[1].fill_between(np.linspace(0, 100, 100),
                 np.nanmean(swing_z_all_mat, axis=0)-np.nanstd(swing_z_all_mat, axis=0),
np.nanmean(swing_z_all_mat, axis=0)+np.nanstd(swing_z_all_mat, axis=0), color='teal', alpha=0.3)
...
for p in range(4):
    ...
    ax[2].fill_between(np.nanmean(swing_y_rel_control_mat[:, p, :], axis=0),
                       ...
                       alpha=0.3)
    ax[2].plot(np.nanmean(swing_y_rel_all_mat[:, p, :], axis=0), np.nanmean(swing_x_rel_all_mat[:, p, :], axis=0), color='teal', linewidth=2)
    ax[2].fill_between(np.nanmean(swing_y_rel_all_mat[:, p, :], axis=0),
                       ...
                       np.nanmean(swing_x_rel_all_mat[:, p, :], axis=0) + np.nanstd(swing_x_rel_all_mat[:, p, :], axis=0), color='teal',
                       alpha=0.3)
...
plt.savefig('E:\\Miniscope processed files\\tied belt locomotion analysis\\trajectories.png')
