...
def detrendROI(rawdata, acq_fq):
    ...
    Ev_Onset, Ev_ApproxPeak, dff_std, IncremSet = Detect_PosEvents_ROI(dff, acq_fq, rtau=0.02, graph=None)
    return [dff, dff_std, Baseline]
...
def DerivGauss_NoiseEstim(data, thres=2):
    ...
    diff = np.diff(data)
    diff_nonan = inpaint_nans(diff)
    # Find outliers using MAD and neutralize them:-
    diff_notzero_idx = np.where(diff != 0)[0]
    diff_notzero = diff_nonan[diff_notzero_idx]
    MAD = robust.mad(diff_notzero, c=0.3)
...
def SlopeThreshold(Data, AmplThres, TimePntThres, CollapSeq=True, acausal=False, verbose=0, graph=None):
    # Data: numpy array (float32!!!) containing the y-data of the time series to be analyzed.
...
<<<<<<< HEAD
def event_detection_calcium_trace(Data, JoinedPosSet, TimePntThres):
    ...
    for i in range(len(JoinedPosSet)):
        peak_idx = np.argmax(Data[JoinedPosSet[i][0]:JoinedPosSet[i][0] + TimePntThres * 2])
        peaks.append(JoinedPosSet[i][0] + peak_idx)
...
def ExtremePairs_Fvals_Rejection(Trajectory, F_vals, AmpThres=0.99, WinThres=10):
    ...
    NewTraj = tl.Interpolate_Runs(Traj_Rej, np.nan)
    Mask = np.invert(np.isnan(Traj_Rej))

    # plt.plot(NewTraj, color='black', marker='o', markersize=3); plt.plot(Trajectory, color='grey', alpha=0.5)
    # plt.scatter(PosExtrPnts, np.take(Trajectory, PosExtrPnts), color='red')
    # plt.scatter(NegExtrPnts, np.take(Trajectory, NegExtrPnts), color='blue')
    # plt.show()
...
def F_val_Ch_detect(F_Crossings, TimePntThres, CollapSeq=True, translate=True, verbose=0):
    ...
    Pos_Intervals = tl.find_runs(OnlyPosSigns, 1)
    Neg_Intervals = tl.find_runs(OnlyNegSigns, 1)
...
def Plot_SlopeTh_Output(Data, PosEnvel, NegEnvel, JoinedPosSet, JoinedNegSet, F_Values, addgraphs=[]):
    # addgraphs: List of graphs to add taht contain elements in the follwing structure:
    ...
    diffData = tl.Normalize(np.append(np.diff(Data), 0),
...
def TestData(type, T, Mult):
   ...
   -0.11492282, -0.43396227, -1.        , -0.97598628, -0.0806175 ,
    ...
    if type == 'sine':
        ...
        filepath, filename, DataEphys, time, Acq_Freq = fl.Load_EphysFile()
        y = DataEphys
        x = np.arange(0,np.size(y))
    elif type == 'simul':
