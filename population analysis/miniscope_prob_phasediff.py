...
for s in range(len(session_data)):
    ...
    # ax[1].set_title('Shuffled calcium events\nfor each trial', fontsize=mscope.fsize - 8)
    plt.savefig(os.path.join(path_session_data, 'Phase difference at CS time - shuffled',
                             animal + '_' + ses_info['protocol'].replace(' ', '_') + '_S' + str(session) + '_phasediff_eventtime_generatedevents'), dpi=mscope.my_dpi)
    # plt.savefig(os.path.join(path_session_data, 'Phase difference at CS time - shuffled',
    #                          animal + '_' + ses_info['protocol'].replace(' ', '_') + '_S' + str(
    #                              session) + '_phasediff_eventtime_shuffledevents'), dpi=mscope.my_dpi)
    ...
    #     bins_pawdiff = np.arange(-1, 1, 0.1)
    ...
    # ax.set_xlabel('FR-FL phase difference', fontsize=mscope.fsize - 4)
