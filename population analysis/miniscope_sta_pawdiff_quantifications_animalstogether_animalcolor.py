...
if session_type == 'split':
    ...
    for count_a, animal in enumerate(animals):
        ...
        for count_c in range(np.shape(sta_zs_rois_bs)[2]):
            if animal != 'MC8855':
                ax.scatter(np.arange(1, 7), sta_zs_rois_bs[:, time_minus100, count_c], s=dotsize, color=get_colors_plot(animal, color_animals))
            if animal == 'MC8855':
                ax.scatter(np.arange(4, 7), sta_zs_rois_bs[:, time_minus100, count_c], s=dotsize, color=get_colors_plot(animal, color_animals))
            ax.scatter(np.arange(7, 17), sta_zs_rois_split[:, time_minus100, count_c], s=dotsize,
                       color=get_colors_plot(animal, color_animals))
            if animal != 'MC9226':
        ...
        for count_c in range(np.shape(sta_zs_rois_bs)[2]):
            if animal == 'MC8855':
                ax.plot(np.arange(4, 27), sta_zs_rois_concat[:, time_minus100, count_c], color=get_colors_plot(animal, color_animals))
            elif animal == 'MC9226':
                ax.plot(np.arange(1, 24), sta_zs_rois_concat[:, time_minus100, count_c], color=get_colors_plot(animal, color_animals))
            else:
                ax.plot(np.arange(1, 27), sta_zs_rois_concat[:, time_minus100, count_c], color=get_colors_plot(animal, color_animals))
    ax.axvline(x=6.5, linewidth=2, linestyle='dashed', color='black')
    ax.axvline(x=16.5, linewidth=2, linestyle='dashed', color='black')
    ...
    for count_a, animal in enumerate(animals):
        ...
        for count_c in range(np.shape(sta_zs_rois_bs)[2]):
            if animal != 'MC8855':
                ax.scatter(np.arange(1, 7), sta_zs_rois_bs[:, time_0, count_c], s=dotsize, color=get_colors_plot(animal, color_animals))
            if animal == 'MC8855':
                ax.scatter(np.arange(4, 7), sta_zs_rois_bs[:, time_0, count_c], s=dotsize, color=get_colors_plot(animal, color_animals))
        ...
        for count_c in range(np.shape(sta_zs_rois_bs)[2]):
            if animal == 'MC8855':
                ax.plot(np.arange(4, 27), sta_zs_rois_concat[:, time_0, count_c], color=get_colors_plot(animal, color_animals))
            elif animal == 'MC9226':
                ax.plot(np.arange(1, 24), sta_zs_rois_concat[:, time_0, count_c], color=get_colors_plot(animal, color_animals))
            else:
                ax.plot(np.arange(1, 27), sta_zs_rois_concat[:, time_0, count_c], color=get_colors_plot(animal, color_animals))
    ax.axvline(x=6.5, linewidth=2, linestyle='dashed', color='black')
    ax.axvline(x=16.5, linewidth=2, linestyle='dashed', color='black')
    ax.set_xlabel('Trials', fontsize=20)
    ...
    ax.spines['top'].set_visible(False)
    ax.tick_params(axis='both', which='major', labelsize=18)
    plt.savefig(os.path.join(save_path, 'sta_bodyvars_FR-FL_displacement_difference_0ms_split_ipsi_S1_animalcolors'), dpi=128)
...
if session_type == 'tied':
    ...
    for count_a, animal in enumerate(animals):
        ...
        for count_c in range(np.shape(sta_zs_rois_bs)[2]):
            ax.scatter((np.ones(np.shape(sta_zs_rois_bs[:, time_minus100, :])[0])*3)+np.random.rand(np.shape(sta_zs_rois_bs[:, time_0, :])[0]), sta_zs_rois_bs[:, time_minus100, count_c], s=dotsize, color=get_colors_plot(animal, color_animals))
            ax.scatter((np.ones(np.shape(sta_zs_rois_fast[:, time_minus100, :])[0])*5)+np.random.rand(np.shape(sta_zs_rois_fast[:, time_0, :])[0]), sta_zs_rois_fast[:, time_minus100, count_c], s=dotsize,
                   color=get_colors_plot(animal, color_animals))
        if animal != 'MC8855':
            ...
            for count_c in range(np.shape(sta_zs_rois_slow)[2]):
                ax.scatter(np.ones(np.shape(sta_zs_rois_slow[:, time_minus100, :])[0]) + np.random.rand(np.shape(sta_zs_rois_slow[:, time_0, :])[0]), sta_zs_rois_slow[:, time_minus100, count_c], s=dotsize,
                       color=get_colors_plot(animal, color_animals))
    ax.set_xticks([1, 3, 5])
    ...
    ax.spines['top'].set_visible(False)
    ax.tick_params(axis='both', which='major', labelsize=18)
    plt.savefig(os.path.join(save_path, 'sta_bodyvars_FR-FL_displacement_difference_-100ms_tied_session_S1_animalcolors'), dpi=128)
    ...
    for count_a, animal in enumerate(animals):
        ...
        sta_zs_rois_fast = np.load(os.path.join(save_path, animal + '_' + protocol, 'sta_bodyvars_FR-FL_displacement_difference_fast.npy'))
        for count_c in range(np.shape(sta_zs_rois_bs)[2]):
            ax.scatter((np.ones(np.shape(sta_zs_rois_bs[:, time_minus100, :])[0])*3)+np.random.rand(np.shape(sta_zs_rois_bs[:, time_minus100, :])[0]), sta_zs_rois_bs[:, time_minus100, count_c], s=dotsize, color=get_colors_plot(animal, color_animals))
            ax.scatter((np.ones(np.shape(sta_zs_rois_fast[:, time_minus100, :])[0])*5)+np.random.rand(np.shape(sta_zs_rois_fast[:, time_minus100, :])[0]), sta_zs_rois_fast[:, time_minus100, count_c], s=dotsize,
                   color=get_colors_plot(animal, color_animals))
        if animal != 'MC8855':
            sta_zs_rois_slow = np.load(
                os.path.join(save_path, animal + '_' + protocol, 'sta_bodyvars_FR-FL_displacement_difference_slow.npy'))
            for count_c in range(np.shape(sta_zs_rois_slow)[2]):
                ax.scatter(np.ones(np.shape(sta_zs_rois_slow[:, time_minus100, :])[0]) + np.random.rand(np.shape(sta_zs_rois_slow[:, time_minus100, :])[0]), sta_zs_rois_slow[:, time_minus100, count_c], s=dotsize,
                       color=get_colors_plot(animal, color_animals))
    ax.set_xticks([1, 3, 5])
    ax.set_xticklabels(['slow', 'baseline', 'fast'], fontsize=20)
    ax.set_xlabel('Belt speed', fontsize=20)
    ...
    ax.spines['top'].set_visible(False)
    ax.tick_params(axis='both', which='major', labelsize=18)
    plt.savefig(os.path.join(save_path, 'sta_bodyvars_FR-FL_displacement_difference_0ms_tied_session_S1_animalcolors'), dpi=128)
