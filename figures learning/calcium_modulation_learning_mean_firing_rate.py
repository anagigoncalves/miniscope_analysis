...
def calcium_rate_modulation_baseline(firing_rate_animal, paw, phase_plot, bs_trial, trials_plot, trials_baseline, trials_split, trials_washout):
    ...
    if trials_plot == "blocks":
        ...
        firing_rate_mod_bs = np.nanmean(firing_rate_mod_all[:, np.int64(trials_idx_selected_bs)], axis=1)
        firing_rate_mod_es = np.nanmean(firing_rate_mod_all[:, np.int64(trials_idx_selected_es)], axis=1)
        firing_rate_mod_ls = np.nanmean(firing_rate_mod_all[:, np.int64(trials_idx_selected_ls)], axis=1)
        firing_rate_mod_ew = np.nanmean(firing_rate_mod_all[:, np.int64(trials_idx_selected_ew)], axis=1)
        firing_rate_mod_lw = np.nanmean(firing_rate_mod_all[:, np.int64(trials_idx_selected_lw)], axis=1)
...
for animal in animals:
    ...
    ax.plot(np.nanmean(firing_rate_FR_st_mod, axis=0), color='orange', linewidth=2, marker='o')
    ax.plot(np.nanmean(firing_rate_FR_sw_mod, axis=0), color='green', linewidth=2, marker='o')
...
ax.plot(np.nanmean(firing_rate_FR_st_mod_all, axis=0), color='orange', linewidth=2, marker='o')
ax.plot(np.nanmean(firing_rate_FR_sw_mod_all, axis=0), color='green', linewidth=2, marker='o')
