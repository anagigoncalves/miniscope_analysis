...
for i in range(len(event_count_loco_slow_all)):
    ...
    plt.scatter(np.repeat(3, len(event_count_loco_baseline_all[i]))+np.random.rand(len(event_count_loco_baseline_all[i])), event_count_loco_baseline_all[i], s=15, color=get_colors_plot(event_count_loco_animals[i], color_animals))
...
plt.savefig(os.path.join(path_session_data, 'cluster_fr_forwardloco_speed'), dpi=mscope.my_dpi)
